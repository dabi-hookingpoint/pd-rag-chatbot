...
BM25 키워드 검색 + Gemini 임베딩(`gemini-embedding-001`, 768차원) 기반 코사인 유사도를 가중합해 검색합니다.

- 청크 임베딩은 `scripts/build_embeddings.py`로 미리 계산해 `public/data/embeddings.json`(정적 벡터스토어, ~320KB)에 저장해 둡니다.
- 사용자 질문만 그때그때 같은 임베딩 모델로 실시간 임베딩해 코사인 유사도를 계산합니다.
- 벡터스토어는 첫 방문 시 다운로드되며, 진행률이 홈 화면에 표시되고 `localStorage`에 캐시되어 다음 방문부터는 즉시 로드됩니다.
- 설정 탭의 슬라이더로 BM25 ↔ 코사인 가중치를 직접 조절할 수 있습니다(기본 50:50).
- API 키가 없거나 벡터스토어가 아직 로딩 중이면 자동으로 BM25만으로 폴백합니다.

## 현재 한계 (정직하게 밝힘)

- **생성 엔진은 Gemini만 지원**합니다(BYOK). OpenAI 등 엔진 선택 기능은 아직 없습니다.
- 근거가 없는 질문은 결합 점수가 임계치(0.12) 미만이면 LLM을 호출하지 않고 사전 규칙으로 즉시 "확인되지 않음"을 안내합니다. 이 임계치는 경험적으로 정한 값이라 더 다듬을 여지가 있습니다.
